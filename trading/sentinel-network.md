...
> **Relationship to Delegated Intent Policy:** RTI is the off-chain behavioral envelope enforced by Baseline. For on-chain fill-time enforcement of analogous constraints (allowed pairs, max slippage, max notional), see the Delegated Intent Policy in `sky-intents.md`. RTI is the first line of defense; DIP provides the on-chain backstop.
...
| `portfolio-halo.md` | lpha-lcts beacon for LCTS-based Portfolio Halos |
| `term-halo.md` | lpha-nfat beacon for NFAT-based Term Halos |
